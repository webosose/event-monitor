...
static GOptionEntry options[] = { { "version", 'v', 0, G_OPTION_ARG_NONE,
        &option_version, "Show version information and exit" }, { nullptr }, };
...
void setupLogging() {
    ...
    if (error != kPmLogErr_None) {
        std::cerr << "Failed to setup up log context " << LOG_CONTEXT_NAME
                << std::endl;
        abort();
    }
}
...
int main(int argc, char **argv) {
    ...
    if (mainLoop == NULL) {
        std::cerr << "Failed to create g_main_loop!" << std::endl;
        exit (EXIT_FAILURE);
    }
